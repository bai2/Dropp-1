...
exports.commands = {
	...
	shadowban: function (target, room, user) {
		if (!target) return this.sendReply("/shadowban o /spam [nick], [razon] - Castiga a un usuario quitandole el derecho a hablar.");
		...
		if (targets.length === 0) {
			return this.sendReply('||' + this.targetUsername + " ya estaba recibiendo su castigo.");
		}
		this.privateModCommand("(" + user.name + " ha castigado a " + targets.join(", ") + (reason ? " (" + reason + ")" : "") + ")");
	...
	unshadowban: function (target, room, user) {
		if (!target) return this.sendReply("/unshadowban o /unspam [username] - Deshace el castigo que se le asigno a un Usuario.");
